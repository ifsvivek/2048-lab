...
const [api, set = 'baseline'] = process.argv.slice(2);
if (!api) throw new Error('usage: submit-results.ts <apiUrl> [set]');
const dir = join(resultsDir, set);
for (const suite of readdirSync(dir).filter((d) => statSync(join(dir, d)).isDirectory())) {
  for (const f of readdirSync(join(dir, suite)).filter((f) => f.endsWith('.json'))) {
