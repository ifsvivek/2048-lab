...
// await shot('replay', '/replay/DPDU-VUTK-7ABC', { act: async (p) => { await p.getByRole('button', { name: 'Play', exact: true }).click(); await p.waitForTimeout(4000); } });
// await shot('runtimes', '/runtimes', { act: async (p) => p.evaluate(() => window.scrollTo(0, 180)) });
